...
export const isPalindrome:(str : string) => boolean = (str : string) => checkIfPilandrome(stringToArray(str).filter(isLetter).map(fromUpperToLower),0,stringToArray(str).filter(isLetter).map(fromUpperToLower).length-1)
...
export const treeToSentence:((tree : WordTree) => string) = (tree : WordTree) => (tree.children.length == 0) ? tree.root : tree.root+" "+tree.children.map(treeToSentence).join(" ")
